...
int cpsr;          // user mode status register
int kregs[16];     // 16 kernel registers

/* Sample interrupt vector table
.data 0xff00     <<< The variable rupt contains 0xff00
.label rupt_tab
mva pc, do_ker   <<< instruction at 0xff00
mva pc, do_tmr   <<< instruction at 0xff04
 */
...
int interrupt(enum rupttype rt) {
    ...
    if (rt == KERNEL) {
        ...
        kregs[KPSR] = tcpsr;
        registers[13] = kregs[KR13]; // swap r13 and kr13
        kregs[KR13] = tr13;
        ...
        cpsr = kregs[IPSR];
        kregs[IPSR] = tcpsr;         // swap cpsr and ipsr
      ...
      case K_INST: // kernel inst
         switch (d->opcode & 0xf) {
           ...
           case SRG: // srg, status reg instruction
               if (!bit_test(cpsr, U)) { // srg instr must be done in kernel/interrupt mode
           ...
           case RFI: // return from interrupt
               ...
               if (!bit_test(cpsr, U) && bit_test(cpsr, OS)) {
                   ...
                   printres("Illegal use of rfi Instruction\n");
                   retval = ILLEGALINST;
               }
               break;
           case MKD: // mkd, move kernel regs as destination
               // must be done in kernel mode with the OS loaded
               if (!bit_test(cpsr, U) && bit_test(cpsr, OS)) {
                   kregs[d->rd] = registers[d->rm];
                   if (d->rd == CPSR)
                       cpsr = kregs[d->rd]; // cpsr variable is used throughout
                   pc += 4;
               }
               else {
                   printres("Illegal use of mkd Instruction\n");
                   retval = ILLEGALINST;
               }
               break;
           case MKS: // mks, move kernel regs as source
               // must be done in kernel mode with the OS loaded
               if (!bit_test(cpsr, U) && bit_test(cpsr, OS)) {
                   if (d->rm == CPSR)
                       kregs[d->rd] = cpsr; // cpsr variable is used throughout
                   registers[d->rd] = kregs[d->rm];
                   pc += 4;
               }
               else {
                   printres("Illegal use of mkd Instruction\n");
